...
class PAM:
    def __init__(self, S, K, alphas, beta, docs, V,
                 smartinit=True, debug=False):
        ...
        for m, doc in enumerate(docs):
            ...
            for t in doc:  # t is id of word
                if smartinit:
                    ...
                    p_zsk = p_s * p_k * p_v
                    """
                    p_zsk = (self.n_m_zk[m] + self.alphask) * self.n_zk_t[:, t] \
    ...
    def hyper_parameter_inference(self):
        ...
        print('alphask_sum', np.sum(self.alphask, axis=1))
        print('alphask', self.alphask)
...
def output_super_sub_topic_dist(pam):
    zsk = np.average(pam.n_m_zk, axis=0)
...
def main():
    ...
    parser.add_option("--debug", dest="debug", action='store_true',
                      help="debug mode(output result of alphas)")
    ...
    pam = PAM(options.S, options.K, options.alphas, options.beta,
              docs, voca.size(), options.smartinit, options.debug)

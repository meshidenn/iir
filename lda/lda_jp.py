...
class LDA:
    def __init__(self, K, alpha, beta, docs, V, smartinit=True):
        ...
        # z is topic, m is docnum, and j is position of vocaburary in doc
        ...
        self.n_z_t = numpy.zeros((K, V)) + beta
        # count of each topic for all vocabulary
        ...
        for m, doc in enumerate(docs):
            ...
            for t in doc:  # t is id of word
                if smartinit:
                    # this cals prevents large number division
                    # so, p_z is just divided by topic-voc
                    # distribution denominator
                    # after that p_z / p_z.sum()
                    p_z = self.n_z_t[:, t] * self.n_m_z[m] / self.n_z
                    z = numpy.random.multinomial(1, p_z / p_z.sum()).argmax()
                else:
                    z = numpy.random.randint(0, K)
                z_j.append(z)  # set topic on each word
                self.n_m_z[m, z] += 1  # set topic count on each doc 
                self.n_z_t[z, t] += 1  # set wordcount on each topic
                self.n_z[z] += 1  # set topic distribution
            self.z_m_j.append(numpy.array(z_j))

    def inference(self):
        """learning once iteration using Gibbs Sampling"""
...
def main():
    ...
    parser.add_option("-i", dest="iteration",
                      ...
                      count", default=100)
    parser.add_option("-s", dest="smartinit",
